...
function ExploreContent() {
  ...
  useEffect(() => {
    const ps = (ProjectStore.getVisibleTo(visRole as 'admin' | 'recruiter' | 'student' | 'public'));
    const is = (IdeaStore.getVisibleTo(visRole as 'admin' | 'recruiter' | 'student' | 'public'));
    const ss = StudentStore.getAll();
    const ts = TeamStore.getAll();
    setProjects(ps); setIdeas(is); setStudents(ss); setTeams(ts);
  }, [visRole]);
    ...
    <div className="page">
      ...
      <main className="main">
          ...
          <div className="container">
            ...
            {showFilters && (
              <div className={styles.filterRow}>
                {DOMAINS.map(d => (
                  <button key={d} className={`${styles.filterChip} ${domain === d ? styles.filterChipActive : ''}`} onClick={() => setDomain(d)}>{d}</button>
                ))}
              </div>
            )}
          </div>
        ...
        <div className="container" style={{ paddingTop: 'var(--space-6)' }}>
          <div className="tabs">
            {([['projects', 'Projects', <Layers key="l" size={14} />], ['ideas', 'Ideas', <Lightbulb key="lb" size={14} />], ['students', 'Students', <User key="u" size={14} />], ['teams', 'Teams', <Users key="us" size={14} />]] as const).map(([t, label, icon]) => (
          ...
          {tab === 'projects' && (
            fp.length > 0
              ? <div className="grid-3">{fp.map(p => <ProjectCard key={p.id} project={p} currentUserId={userId ?? undefined} />)}</div>
              : <div className="empty-state"><Search size={40} /><p className="empty-state__title">No projects found</p><p className="empty-state__body">Try adjusting your search or filters.</p></div>
          )}

          {tab === 'ideas' && (
            fi.length > 0
              ? <div className="grid-3">{fi.map(i => <IdeaCard key={i.id} idea={i} currentUserId={userId ?? undefined} />)}</div>
              : <div className="empty-state"><Lightbulb size={40} /><p className="empty-state__title">No ideas found</p><p className="empty-state__body">Try a different search.</p></div>
          )}

          {tab === 'students' && (
            fs.length > 0
              ? <div className="grid-4">{fs.map(s => <StudentCard key={s.userId} profile={s} />)}</div>
              : <div className="empty-state"><User size={40} /><p className="empty-state__title">No students found</p></div>
          )}

          {tab === 'teams' && (
            ft.length > 0
              ? <div className="grid-3">{ft.map(t => (
                <Link key={t.id} href={`/team/${t.id}`} className="card card-hover" style={{ padding: 'var(--space-5)', display: 'flex', flexDirection: 'column', gap: 'var(--space-3)' }}>
                  <div style={{ display: 'flex', gap: 'var(--space-3)', alignItems: 'center' }}>
                    <img src={t.avatar} alt={t.name} className="avatar avatar-md" />
                    <div>
                      <div style={{ fontWeight: 700, color: 'var(--text-1)' }}>{t.name}</div>
                      <div style={{ fontSize: 'var(--text-xs)', color: 'var(--text-4)' }}>{t.members.length} members</div>
                    </div>
